...
public class SysSecurityLogServiceImpl extends ServiceImpl<SysSecurityLogMapper, SysSecurityLog>
    ...
    @Override
    public Page<SysSecurityLog> listSecurityLogs(BasePageRequest request, String operationType, Long userId,
                                                 LocalDateTime startTime, LocalDateTime endTime) {
        Page<SysSecurityLog> page = new Page<>(request.getPageNum(), request.getPageSize());
        return lambdaQuery()
                .eq(operationType != null && !operationType.isBlank(), SysSecurityLog::getOperationType, operationType)
                .eq(userId != null, SysSecurityLog::getUserId, userId)
                .ge(startTime != null, SysSecurityLog::getOperationTime, startTime)
                .le(endTime != null, SysSecurityLog::getOperationTime, endTime)
                .orderByDesc(SysSecurityLog::getOperationTime)
                .page(page);
    }
}
