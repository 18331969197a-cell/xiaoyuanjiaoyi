...
public class SysDictTypeServiceImpl extends ServiceImpl<SysDictTypeMapper, SysDictType>
    ...
    public boolean refreshCache() {
        try {
            ...
            List<String> existingKeys = cacheProvider.scanKeys(CacheConstants.Dict.DICT_CACHE_PREFIX + "*");
            if (!existingKeys.isEmpty()) {
                long deletedCount = cacheProvider.delete(existingKeys);
            ...
            for (SysDictType dictType : dictTypes) {
                try {
                    ...
                    // 将数据放入缓存
                    String cacheKey = String.format(CacheConstants.Dict.DICT_DATA_KEY, dictType.getDictType());
                    cacheProvider.set(cacheKey, options, CacheConstants.Dict.DICT_CACHE_EXPIRE_TIME, TimeUnit.SECONDS);
    ...
    private void evictDictOptions(String dictType) {
        if (!StringUtils.isBlank(dictType)) {
            try {
                ...
                String manualKey = String.format(CacheConstants.Dict.DICT_DATA_KEY, dictType);
                cacheProvider.delete(manualKey);
            } catch (Exception e) {
