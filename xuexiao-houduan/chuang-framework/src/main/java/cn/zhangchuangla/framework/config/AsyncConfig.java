...
public class AsyncConfig {
     ...
     * QC任务核心线程数 - 数据处理任务
     */
    private static final int QC_CORE_POOL_SIZE = 2;

    /**
     * QC任务最大线程数 - 限制并发QC任务数量
     */
    private static final int QC_MAX_POOL_SIZE = 5;

    /**
     * QC任务队列容量 - 缓存待处理QC任务
     */
    private static final int QC_QUEUE_CAPACITY = 100;
    ...
    public Executor qcTaskExecutor() {
        ...
        executor.setCorePoolSize(QC_CORE_POOL_SIZE);
        executor.setMaxPoolSize(QC_MAX_POOL_SIZE);
        executor.setQueueCapacity(QC_QUEUE_CAPACITY);
        executor.setKeepAliveSeconds(DEFAULT_KEEP_ALIVE_SECONDS);
        ...
        executor.initialize();
        log.info("QC任务异步线程池初始化完成 - 核心线程数: {}, 最大线程数: {}",
                executor.getCorePoolSize(), executor.getMaxPoolSize());
