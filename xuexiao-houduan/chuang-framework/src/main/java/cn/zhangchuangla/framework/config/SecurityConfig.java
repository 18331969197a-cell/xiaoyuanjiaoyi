...
public class SecurityConfig {
    ...
    /**
     * 配置完全忽略安全检查的路径 - 上传文件访问
     * 这个过滤器链优先级最高，专门处理静态资源
     */
    @Bean
    @Order(0)
    ...
    /**
     * 配置WebSocket端点的安全过滤器链
     * 允许WebSocket握手请求通过，包括SockJS的各种回退路径（xhr, xhr_streaming, jsonp等）
     */
    @Bean
    @Order(1)
    public SecurityFilterChain websocketSecurityFilterChain(HttpSecurity http) throws Exception {
        return http
                ...
                .anonymous(anonymous -> anonymous.principal(RolesConstant.ANONYMOUS))
                .build();
    }

    /**
     * 完全忽略WebSocket路径的安全检查
     * 这是最彻底的方式，让Spring Security完全不处理这些请求
     */
    @Bean
