...
public class SessionService {
    ...
    public PageResult<OnlineLoginUserVo> sessionList(OnlineUserQueryRequest request) {
        String accessTokenKeyPattern = CacheConstants.Auth.USER_ACCESS_TOKEN + "*";
        List<OnlineLoginUser> sessionList = new ArrayList<>();
        Map<String, Object> stringObjectMap = cacheProvider.scanKeysWithValues(accessTokenKeyPattern);
    ...
    public OnlineLoginUser sessionDetail(String accessTokenId) {
        ...
        return cacheTokenStore.getAccessToken(accessTokenId);
    }
    ...
    public boolean deleteSession(String accessTokenId) {
        Assert.isTrue(cacheTokenStore.isValidAccessToken(accessTokenId), "会话ID不存在!");
        String refreshTokenId = cacheTokenStore.getRefreshTokenIdByAccessTokenId(accessTokenId);
        //删除设备信息
        deviceService.deleteDevice(refreshTokenId);
        //删除会话信息
        cacheTokenStore.deleteAccessToken(accessTokenId);
        cacheTokenStore.deleteRefreshToken(refreshTokenId);
        return true;
    }
