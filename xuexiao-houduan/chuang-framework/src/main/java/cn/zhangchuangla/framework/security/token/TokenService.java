...
public class TokenService {
    ...
    public LoginSessionDTO createToken(Authentication authentication, LoginDeviceDTO loginDeviceDTO) {
        ...
        cacheTokenStore.setRefreshToken(refreshTokenSessionId, accessTokenSessionId);
        cacheTokenStore.setAccessToken(accessTokenSessionId, onlineLoginUser);
    ...
    public Authentication parseAccessToken(String accessToken) {
        ...
        OnlineLoginUser onlineUser = cacheTokenStore.getAccessToken(accessTokenId);

        //更新访问时间
        boolean updateSuccess = cacheTokenStore.updateAccessTime(accessTokenId);
        if (!updateSuccess) {
    ...
    public boolean validateRefreshToken(String jwtRefreshToken) {
        ...
        return cacheTokenStore.isValidRefreshToken(refreshTokenSessionId);
    }
    ...
    public AuthTokenVo refreshToken(String jwtRefreshToken) {
        ...
        if (!cacheTokenStore.isValidRefreshToken(refreshTokenSessionId)) {
            throw new AuthorizationException(ResultCode.REFRESH_TOKEN_INVALID);
        }
        ...
        cacheTokenStore.mapRefreshTokenToAccessToken(refreshTokenSessionId, accessTokenSessionId);
        // 返回新的访问令牌
