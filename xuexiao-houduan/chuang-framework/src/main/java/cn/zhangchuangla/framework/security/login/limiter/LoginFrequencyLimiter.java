...
public class LoginFrequencyLimiter {
    ...
    private final LoginFrequencyStore loginFrequencyStore;
    ...
    public void recordLoginSuccess(String username) {
        ...
        // 增加小时成功次数
        loginFrequencyStore.incrementHourlySuccessCount(username, getSecondsToNextHour());

        // 增加每日成功次数
        loginFrequencyStore.incrementDailySuccessCount(username, getSecondsToNextDay());
    ...
    private void checkHourlyLimit(String username, int maxLoginPerHour) {
        int successCount = loginFrequencyStore.getHourlySuccessCount(username);

        if (successCount >= maxLoginPerHour) {
            long remainingSeconds = loginFrequencyStore.getHourlyExpireSeconds(username);
    ...
    private void checkDailyLimit(String username, int maxLoginPerDay) {
        int successCount = loginFrequencyStore.getDailySuccessCount(username);

        if (successCount >= maxLoginPerDay) {
            long remainingSeconds = loginFrequencyStore.getDailyExpireSeconds(username);
            String message = remainingSeconds > 0
    ...
    public int getTodayLoginCount(String username) {
        Assert.hasText(username, "用户名不能为空");
        return loginFrequencyStore.getDailySuccessCount(username);
    }
    ...
    public int getHourlyLoginCount(String username) {
        Assert.hasText(username, "用户名不能为空");
        return loginFrequencyStore.getHourlySuccessCount(username);
    }
}
