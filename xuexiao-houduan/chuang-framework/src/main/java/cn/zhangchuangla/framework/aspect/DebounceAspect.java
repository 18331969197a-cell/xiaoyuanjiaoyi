...
public class DebounceAspect {
    ...
    private static final String DEBOUNCE_IP = "debounce:ip:";
    private static final String DEBOUNCE_USER = "debounce:user:";
    private static final String DEBOUNCE_CUSTOM = "debounce:custom:";
    ...
    public Object around(ProceedingJoinPoint joinPoint, Debounce debounce) throws Throwable {
        ...
        String debounceKey = buildDebounceKey(attributes, joinPoint, type, effective.key());

        try {
            boolean allowed = debouncer.tryAcquire(debounceKey, windowMillis);
            if (!allowed) {
        ...
        switch (type) {
            case USER -> {
                try {
                    ...
                    key.append(DEBOUNCE_USER).append(baseKey).append(":").append(user.getUserId());
                } catch (Exception e) {
                    String ip = request != null ? IPUtils.getIpAddress(request) : "non-web";
                    key.append(DEBOUNCE_IP).append(baseKey).append(":").append(ip);
            ...
            case CUSTOM -> {
                ...
                key.append(DEBOUNCE_CUSTOM).append(baseKey).append(":").append(evaluated);
            }
            default -> {
                String ip = request != null ? IPUtils.getIpAddress(request) : "non-web";
                key.append(DEBOUNCE_IP).append(baseKey).append(":").append(ip);
