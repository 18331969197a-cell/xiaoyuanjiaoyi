...
public class MessageProducer {
    ...
    public MessageProducer(@Nullable RabbitTemplate rabbitTemplate) {
    ...
    public void sendUserMessage(MessageSendDTO messageSendDTO) {
        if (rabbitTemplate == null) {
            log.warn("RabbitMQ未启用，跳过发送用户消息，消息ID: {}", messageSendDTO.getMessageId());
            return;
        }
        try {
    ...
    public void sendRoleMessage(MessageSendDTO messageSendDTO) {
        if (rabbitTemplate == null) {
            log.warn("RabbitMQ未启用，跳过发送角色消息，消息ID: {}", messageSendDTO.getMessageId());
            return;
        }
        try {
            if (messageSendDTO == null) {
    ...
    public void sendDeptMessage(MessageSendDTO messageSendDTO) {
        if (rabbitTemplate == null) {
            log.warn("RabbitMQ未启用，跳过发送部门消息，消息ID: {}", messageSendDTO.getMessageId());
            return;
        }
        try {
            if (messageSendDTO == null) {
