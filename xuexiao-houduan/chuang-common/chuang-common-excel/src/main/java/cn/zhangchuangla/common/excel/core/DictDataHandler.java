...
public class DictDataHandler {
    ...
    public String getDictLabel(String dictKey, String dictValue) {
        ...
        try {
            ...
            if (dictMap == null) {
                // 从缓存获取字典数据
                dictMap = loadDictFromCache(dictKey);
     ...
     * @return 字典值到标签的映射
     */
    private Map<String, String> loadDictFromCache(String dictKey) {
        try {
            String cacheKey = String.format(CacheConstants.Dict.DICT_DATA_KEY, dictKey);
            List<Option<String>> dictValues = cacheProvider.get(cacheKey);

            if (dictValues == null || dictValues.isEmpty()) {
                log.debug("缓存中未找到字典数据: dictType={}", dictKey);
                return null;
            }
            ...
            log.debug("从缓存加载字典数据成功: dictType={}, size={}", dictKey, dictMap.size());
            return dictMap;
        } catch (Exception e) {
            log.error("从缓存加载字典数据失败: dictType={}", dictKey, e);
            return null;
        }
    }
    ...
    public void preloadDictData(List<String> dictKeys) {
        ...
        for (String dictKey : dictKeys) {
            if (StringUtils.isNotBlank(dictKey) && !localCache.containsKey(dictKey)) {
                Map<String, String> dictMap = loadDictFromCache(dictKey);
                if (dictMap != null && !dictMap.isEmpty()) {
                    localCache.put(dictKey, dictMap);
                }
            }
