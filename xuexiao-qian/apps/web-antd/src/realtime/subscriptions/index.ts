...
import { registerLostfoundNotificationSubscription } from './lostfound-notification';
...
export function registerAppSubscriptions() {
  ...
  // 注册失物招领通知订阅
  const lostfoundCleanup = registerLostfoundNotificationSubscription(service);
  subscriptionCleanupFunctions.push(lostfoundCleanup);
}
